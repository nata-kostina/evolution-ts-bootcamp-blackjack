...
export class MultiPlayersController implements Controller {
  ...
  public async handleInitGame({
    ...
    try {
      ...
      const roomID = availableRoom || this._gameStore.createNewRoom();
      ...
      const game = this._gameStore.getGame(roomID);
      let sessionManager = this.sessionsManagers.find((session) => session.roomID === roomID);
      if (!sessionManager) {
        sessionManager = new GameSessionManager(roomID, this._respondManager, this._gameStore, this._playersStore, io);
        this.sessionsManagers.push(sessionManager);
      }
      sessionManager.addPlayer({ playerID: id, socket });
  ...
  public async handlePlaceBet({
    ...
    try {
      ...
      this._respondManager.respondWithDelay({
        roomID: socketID,
        event: 'placeBet',
